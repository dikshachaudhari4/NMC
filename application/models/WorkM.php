...
class WorkM extends CI_Model
{
        ...
        if ($this->db->set('active', 0)->update('popup_data')) {
            if ($this->db->set('active', 1)->where_in('id', $id)->update('popup_data')) {
                $logs = [
                    ...
                    "description" => "Popup Actived by " . $this->session->username . " in " . "In a Popup Section" . " Section.",
                ];
                $this->db->insert('logs', $logs);
    ...
    public function GetLogs($k, $limit, $offset = 0) //To retrive all data upto limit as per category

    {
        $query = $this->db->order_by('time', 'DESC')->limit($limit)->get($k);
        $result = $query->result();
        return $result;
    }
...

    {
        if ($this->db->insert($tablename, $data)) {
            ...
            //     // return false;
            // }
            if ($tablename == 'our_attractions' or $tablename == 'exploring_nashik_dest' or $tablename == 'exploring_nashik_dest' or $tablename == 'museums_dest' or $tablename == 'mountains_dest' or $tablename == 'waterfalls_dest' or $tablename == 'food-stuff_home' or $tablename == 'food-stuff_sec4' or $tablename == 'food-stuff_sec3' or $tablename == 'our_events_culture' or $tablename == 'places_for_worship_dest') {
...

    {
        ...
        $d = $query->result();
        if ($query->num_rows()) {
            if (password_verify($pw, $d[0]->password) && $d[0]-> verified) {

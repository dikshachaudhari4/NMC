...
class UserC extends CI_Controller
{
...

    {
        ...
        if ($this->check($k)) {
            ...
            $UserData = [];
            if ($k == "logs"){
                $UserData = $this->WorkM->GetLogs($k,10);
            ...
            $this->load->view('private/base', compact('Page'));} else {
            echo $k;
            show_404();
        }
    }
    ...
    public function changedOrder($k)
    {
        $this->load->model('WorkM');
        if (isset($_POST['orderby'])) {
            // echo "<pre>";
            // print_r($_POST);
            $data = $this->WorkM->changedOrder($k, $_POST['orderby']);
...

    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($k == 'popup_data') {if (isset($_POST['status'])) {
                // popup will display
...
';
...

    {
        $config = array(
            'protocol' => 'smtp',
            'smtp_host' => 'ssl://mail.nmc.gov.in',
            'smtp_port' => 465,
            'smtp_user' => 'info@tourism.nmc.gov.in',
            'smtp_pass' => 'EDz#&2(gYoGR',
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n",
            'wordwrap' => true,
